 ...
 * \invia_beginReg
 * 0     & DEM\_ACCESS         & '0' & '0' & R/W   & '1' : The $SW$ has the hand to enable or disable the demodulators
 *(TEST mode)\\
 ...
 *       &                     &     &     &       & '01' : retro='1' (TEST mode or power saving configuration when $HF$
 * not used)  \\
 *       &                     &     &     &       & '10' : retro=$Fc$/16 (TEST mode)\\
 *       &                     &     &     &       & '11' : retro=$Fc$/8 (TEST mode) \\\hline
 * 11:10 & RFU                 &  0  &  0  & R/-   & RFU \\ \hline
 * 12    & CLK\_EXTR\_DIS      &  0  &  0  & R/W   & When '1', the clock extractor of the Analog Front end is disabled
 *(power saving configuration when $HF$ not used) \\ \hline
 ...
 * 2     & REQA\_IGNORE        & '0'   & '0'   & R/W   & The SW shall set this bit to '1' after the first valid $RATS$
 * or C-$RATS$  \\
 *       &                     &       &       &       & '0': a BACK\_TO\_HALT will make the HW $FSM$ in IDLE state
 *(like after $POR$). \\
 *       &                     &       &       &       & '1': a BACK\_TO\_HALT will make the HW $FSM$ in HALT state. \\
 *\hline
 * 3     & TYPEA\_L4\_ENA      & '0'   & '0'   & R/W   & The SW shall set this bit to '1' when entering in ISO Type A
 * Layer 4 \\
 *       &                     &       &       &       & The SW shall set this bit to '0' after a valid DESELECT command
 *\\
 *       &                     &       &       &       & '0': only 64 bytes of the IO $RAM$ buffer can be filled during
 *$Rx$ period \\
 *       &                     &       &       &       & '1': the 1 kbytes of the IO $RAM$ can be filled during the $Rx$
 * period  \\ \hline
 ...
 * \invia_beginReg
 * 0     & WAIT\_RX            & '0'   & '0'   & R/W   & When STATUS.COM\_INFO='00', launch the $HF$ digital controller
 * for Type A Layer 3 handling  \\
 ...
 *       &                     &       &       &       & This bit shall not be launched when STATUS.COM\_INFO='01' or
 *'10' \\
 *       &                     &       &       &       & Note that at each WAIT\_RX='1', the bit rates configurations
 * are reloaded inside the digital controller \\
 *       &                     &       &       &       & Set Only by SW, reset by HW\\ \hline
 * 1     & LAUNCH\_TX          & '0'   & '0'   & R/W   & Launch the $Tx$ frame with the $IO$ $RAM$ buffer content after
 * waiting SILENT\_TIME slots \\
 *       &                     &       &       &       & Set Only by SW, reset by HW\\ \hline
 * 3:2   & RFU                 &  0    &  0    & R/-   & RFU                                  \\ \hline
 ...
 * state immediatly\\
 *       &                     &       &       &       & - If launched with LAUNCH\_TX: the HALT or IDLE  state will be
 * reached after the end of the $Tx$ frame \\
 ...
 * 7     & CRC\_RAM            & '0'   & '0'   & R/W   & '1' : Tx only. No $CRC$ bytes added at end of frame, final size
 * is  TX\_RAM\_FRAME\_SIZE\\
 *       &                     &       &       &       & '0' : Tx only. The $FSM$ generates on the fly the $CRC$ from
 * the data information present inside the $IO$ $RAM$ buffer\\ \hline
 ...
 *       &                     &      &      &     & This status is set to'0' when in 1) COM\_INFO='00' (BOOT), or 2)
 * COM\_INFO='11' ($Exec$ period) during ISO Type A Layer 4 \\
 ...
 * feature activated.\\
 *       &                     &      &      &     & It is only updated with the presence of the $SClk$ \\
 *       &                     &      &      &     & It is automatically set by $HW$, clearable by $SW$ by writing '1'
 *\\ \hline
 ...
 *       & \_ERR               &      &      &     & two bytes, done in the whole byte of the frame (ISO type A layer 4)
 * in $Rx$ \\
 *       &                     &      &      &     & When '1', a $CRC$ error has been detected\\
 *       &                     &      &      &     & Updated when $Rx$ to $Exec$ period transition \\ \hline
 * 8     & RAM\_OVERF          & '0'  & '0'  & R/- & $IO$ $RAM$ overflow error flag \\
 *       & \_ERR               &      &      &     & When '1', the complete 1KB $IO$ $RAM$ buffer has been filled by the
 *$Rx$ frame \\
 *       &                     &      &      &     & Updated when $Rx$ to $Exec$ period transition\\ \hline
 ...
 *       &                     &      &      &     & When '1', the Slot counter has reached the value of the watchpoint
 * in $Exec$ period\\
 *       &                     &      &      &     & Clear only by $SW$ by writing '1' \\ \hline
 * 15:14 & RFU                 &  0   &  0   & R/- & RFU \\ \hline
 ...
 *       &                     &      &      &       & This selection influences the digital encodeur subcarrier when
 * modulation $BPSK$ $NRZ$-L \\
 *       &                     &      &      &       & This selection influences the parity bit and generation ($Tx$)
 *\\\hline
 ...
 *       &                     &       &        &       & When '1': allows to count with $Fc$ when \texttt{sda\_a} is
 * down \\ \hline
 * 31:29 & RFU                 &  0    &  0     & R/-   & RFU                                  \\
 * \invia_endReg
 */
 ...
 * 7:4    & RFU                 &   0   &  0    & R/-   & RFU
 *                                                                \\ \hline
 * 8      & HF\_RESET\_CFG      &  '0'  & '0'   & R/W   & When '1', a $HF$ reset will be triggered at each RF\_ON event
 *      \\ \hline
...
static inline void _isohf_setDigitalCntCfg(HFCTRL isohf, uint32_t CounterIndex, uint32_t CntValue)
{
    ...
    /* HW_write32 (&isohf[HFCTRL_DIGITAL_CNT0_CFG + (CounterIndex>>1)],
     *    (isohf[HFCTRL_DIGITAL_CNT0_CFG + (CounterIndex>>1)] & HF_DIG_CNT0_MASK) | ((CntValue<<HF_DIG_CNT1_SHIFT) &
     * HF_DIG_CNT1_MASK), 2); */
}
